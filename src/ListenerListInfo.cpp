...
ListenerListInfo::~ListenerListInfo() {
  for (std::vector<struct addrinfo *>::iterator it = listener_records_.begin();
       it != listener_records_.end(); ++it)
    freeaddrinfo(*it);
}

void ListenerListInfo::AddDefaultsRecords() { AddRecords(NULL, kDefaultPort); }
...
void ListenerListInfo::CreateListeners(std::vector<Listener *> &listeners) {
  ...
  for (std::vector<struct addrinfo *>::const_iterator it =
           listeners_addr_uniq.begin();
       it != listeners_addr_uniq.end(); ++it) {
...
bool ListenerListInfo::IsValidUniqAddr(struct addrinfo *addr) const {
  ...
  for (std::vector<struct addrinfo *>::const_iterator it =
           listeners_addr_uniq.begin();
       it != listeners_addr_uniq.end(); ++it)
    if (addr->ai_family == (*it)->ai_family &&
        memcmp(addr->ai_addr, (*it)->ai_addr, sizeof(struct sockaddr)) == 0)
      return false;
  return true;
}
