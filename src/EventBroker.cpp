...
EventBroker::EventBroker(const std::vector<Listener *> &listeners)
  ...
  for (size_t i = 0; i < listeners_.size(); ++i)
    EV_SET(&events[i], listeners_[i]->get_sfd(), EVFILT_READ, EV_ADD, 0, 0,
           NULL);
...
EventBroker::EventBroker(const std::vector<Listener *> &listeners)
  ...
  for (std::vector<Listener *>::const_iterator it = listeners_.begin();
    ...
    if (epoll_ctl(queue_, EPOLL_CTL_ADD, event.data.fd, &event) == -1) {
      // TODO: or perror ?
      for (std::vector<Listener *>::const_iterator it2 = listeners_.begin();
           it2 != it; ++it2)
...
EventBroker::~EventBroker() {
  ...
  for (std::vector<Listener *>::const_iterator it = listeners_.begin();
       it != listeners_.end(); ++it, ++it_events)
    EV_SET(&(*it_events), (*it)->get_sfd(), EVFILT_READ, EV_DELETE, 0, 0, NULL);
  for (std::vector<int>::const_iterator it = accepted_sfd_list_.begin();
       it != accepted_sfd_list_.end(); ++it, ++it_events)
...
int EventBroker::run() {
  ...
  while (g_signal_received == 0) {
    ...
    number_events =
        kevent(queue_, NULL, 0, event, kMaxEvents, NULL /* TODO: timeout ? */);
    // TODO: what happens if kevent fails?
    ...
    for (int i = 0; i < number_events && g_signal_received == 0; ++i) {
      if (IsListener(event[i].ident)) {
        ...
        std::cout << "Received data from socket " << event[i].ident
                  << std::endl;
        char buf[10000];
        int bytes_read =
            recv(event[i].ident, buf, sizeof(buf) - 1, 0 /* MSG_WAITALL */);
        buf[bytes_read] = 0;
...
int EventBroker::run() {
  ...
  while (g_signal_received == 0) {
    std::cout << "Waiting for events..." << std::endl;
    number_events =
        epoll_wait(queue_, event, kMaxEvents, -1 /* TODO: timeout ? */);
    ...
    for (int i = 0; i < number_events && g_signal_received == 0; ++i) {
      if (IsListener(event[i].data.fd)) {
        ...
        // TODO: for now, just echo back the request
        std::cout << "Received data from socket " << event[i].data.fd
                  << std::endl;
        char buf[10000];
        int bytes_read =
            recv(event[i].data.fd, buf, sizeof(buf) - 1, 0 /* MSG_WAITALL */);
        buf[bytes_read] = 0;
        const std::string response =
            "HTTP/1.1 200 OK\r\nContent-Length: 12\r\n\r\nHello world!";
...
int EventBroker::AcceptConnection(int ident) {
  ...
  int new_sfd =
      accept(ident, reinterpret_cast<struct sockaddr *>(&addr), &addr_len);
  if (new_sfd == -1) {
  ...
  // TODO: check if push_back fails, and may be closes the socket, and delete
  // from queue
...
int EventBroker::DeleteConnection(int ident) {
  ...
  close(ident);
  accepted_sfd_list_.erase(
      std::find(accepted_sfd_list_.begin(), accepted_sfd_list_.end(), ident));
